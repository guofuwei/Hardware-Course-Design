...
//	if((x+width-1)>xLCD.width)return PIC_WINDOW_ERR;		//区域错误
//	if((y+height-1)>xLCD.height)return PIC_WINDOW_ERR;	//区域错误 
// 	 
//#if BMP_USE_MALLOC == 1	//使用malloc	
//	databuf=(uint16_t*)pic_memalloc(1024);		//开辟至少bi4width大小的字节的内存区域 ,对240宽的屏,480个字节就够了.
//	if(databuf==NULL)return PIC_MEM_ERR;		//内存申请失败.
//	f_bmp=(FIL *)pic_memalloc(sizeof(FIL));	//开辟FIL字节的内存区域 
//	if(f_bmp==NULL)								//内存申请失败.
//	{		 
